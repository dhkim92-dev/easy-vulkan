...
public:
    ...
    void setup_descriptor_pool() {
        ...
        ev::logger::Logger::getInstance().debug("[Setup Descriptor Sets End]");
    }

    void setup_descriptor_set_layouts() {
        ev::logger::Logger::getInstance().debug("[Setup Descriptor Set Layouts Start]");
        ...
        ev::logger::Logger::getInstance().debug("[Setup Descriptor Set Layouts End]");
    }
    ...
    void setup_descriptor_set() {
        ...
        for (size_t i = 0; i < descriptors.sets.size(); ++i) {
            ...
            if (!descriptors.sets[i]) {
                ev::logger::Logger::getInstance().error("Failed to allocate descriptor set");
                return;
            }
        }
        ev::logger::Logger::getInstance().debug("[Setup Descriptor Sets End]");
    ...
    void setup_pipeline_layouts() {
        ev::logger::Logger::getInstance().debug("[Setup Pipeline Layouts Start]");
        ...
        ev::logger::Logger::getInstance().debug("[Setup Pipeline Layouts] descriptor set layout size: " +  std::to_string(descriptors.layouts.size()));
        ...
        ev::logger::Logger::getInstance().debug("[Setup Pipeline Layouts End]");
    }
    ...
    void setup_camera() {
        ev::logger::Logger::getInstance().debug("[Setup Camera Start]");
        ...
        ev::logger::Logger::getInstance().debug("[Setup Camera End]");
    }
    ...
    void record_command_buffers() {
        ev::logger::Logger::getInstance().debug("[Record Command Buffers Start]");
        ...
        command_buffers[current_frame_index]->bind_descriptor_sets(VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layouts[0], {descriptors.sets[current_frame_index]}, 0);
        command_buffers[current_frame_index]->bind_graphics_pipeline(graphics_pipeline);
        command_buffers[current_frame_index]->bind_push_constants(
            pipeline_layouts[0], 
        ...
        gltf_model->draw(command_buffers[current_frame_index],
    ...
    void setup_command_buffers() {
        ev::logger::Logger::getInstance().debug("[Setup Command Buffers Start]");
        ...
        ev::logger::Logger::getInstance().debug("[Setup Command Buffers End]");
    }
    ...
    void setup_shaders() {
        ...
        ev::utility::read_spirv_shader_file( (shader_path / this->title / "mesh.vert.spv").c_str(), vertex_shader_code);
        ...
        ev::utility::read_spirv_shader_file( (shader_path / this->title / "mesh.frag.spv").c_str(), fragment_shader_code);
    ...
    void setup_graphics_pipeline() {
        ...
        if (!info) {
            ev::logger::Logger::getInstance().error("Failed to get vertex input state");
            exit(EXIT_FAILURE);
        }
