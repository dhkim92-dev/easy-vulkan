...
public:
    ...
    void record_command_buffers() {
        ...
        command_buffers[current_buffer_index]->bind_descriptor_sets(VK_PIPELINE_BIND_POINT_GRAPHICS, 
            ...
            {descriptors.cube_sets[current_buffer_index]});
    ...
    void setup_descriptor_sets() {
        ...
        for (uint32_t i = 0 ; i < swapchain->get_images().size(); ++i) {
            ...
            ev_log_debug("Cube Descriptor set %d allocated.", i);
            descriptors.cube_sets.back()->write_buffer(
                0,
                d_buffers.camera_uniform,
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
            );
            ev_log_debug("Cube Descriptor set %d Camera Perspective View uniform buffer written", i);
            ...
            ev_log_debug("Cube Descriptor set %d created", i);
        }
        ...
        ev_log_debug("Light descriptor set layout created");
        for (uint32_t i = 0 ; i < swapchain->get_images().size(); ++i) {
            descriptors.light_sets.push_back(descriptor_pool->allocate(descriptors.light_layout));
            ev_log_debug("Light Descriptor set %d allocated.", i);
            ...
            ev_log_debug("Light descriptor set %d created", i);
        }
    }
    ...
    void render() override {
     ...
     void pre_destroy() override {
         ...
         ev_log_debug("Pre-destroy cleanup for Light Cube example");
         queue->wait_idle(UINT64_MAX);
         ev_log_debug("Pre-destroy cleanup for Light Cube example complete");
     }
};
