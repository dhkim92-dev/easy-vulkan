...
private:
    ...
    struct {
        ...
        std::vector<std::shared_ptr<ev::PipelineLayout>> cube_layouts;
        std::vector<std::shared_ptr<ev::GraphicsPipeline>> cube_pipelines;
        std::vector<std::shared_ptr<ev::PipelineLayout>> light_layouts;
        std::vector<std::shared_ptr<ev::GraphicsPipeline>> light_pipelines;
    } pipelines;
    ...
    struct {
        ...
        std::shared_ptr<ev::DescriptorSetLayout> cube_layout;
        std::vector<std::shared_ptr<ev::DescriptorSet>> cube_sets;
        std::shared_ptr<ev::DescriptorSetLayout> light_layout;
        std::vector<std::shared_ptr<ev::DescriptorSet>> light_sets;
    } descriptors;
    ...
    const std::vector<Vertex> h_vertices = {
        ...
        // pos(vec3), normal(vec3), tex_coord(vec2)

        // 앞면 (z=0.5)
        {glm::vec3(-0.5f, -0.5f, 0.5f), glm::vec3(0.0f, 0.0f, 1.0f),   glm::vec2(0.0f, 0.0f)}, // 0 좌하
        {glm::vec3(0.5f, -0.5f, 0.5f), glm::vec3(0.0f, 0.0f, 1.0f), glm::vec2(1.0f, 0.0f)},  // 1 우하
        {glm::vec3(0.5f, 0.5f, 0.5f), glm::vec3(0.0f, 0.0f, 1.0f),   glm::vec2(1.0f, 1.0f)},   // 2 우상
        {glm::vec3(-0.5f, 0.5f, 0.5f), glm::vec3(0.0f, 0.0f, 1.0f),  glm::vec2(0.0f, 1.0f)},  // 3 좌상
        ...
        {glm::vec3(0.5f, 0.5f, -0.5f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec2(1.0f, 1.0f)},  // 15
    ...
    struct {
        ...
        std::shared_ptr<ev::Shader> light_vertex;
        std::shared_ptr<ev::Shader> light_fragment;
    } shaders;
...
public:
    ...
    void setup_uniform_buffer() {
        ...
        // Device Camera Uniform Buffer set
        ...
        ev::logger::Logger::getInstance().debug("Light uniform buffer handler : " + std::to_string(reinterpret_cast<uintptr_t>(light_uniform_buffer_handle)));
    ...
    void record_command_buffers() {
        ...
        command_buffers[current_frame_index]->bind_descriptor_sets(VK_PIPELINE_BIND_POINT_GRAPHICS, 
            pipelines.cube_layouts[0], 
            {descriptors.cube_sets[current_frame_index]});
        command_buffers[current_frame_index]->bind_graphics_pipeline(pipelines.cube_pipelines[0]);
        ...
        command_buffers[current_frame_index]->bind_descriptor_sets(VK_PIPELINE_BIND_POINT_GRAPHICS, 
            ...
            {descriptors.light_sets[current_frame_index]});
        command_buffers[current_frame_index]->bind_graphics_pipeline(pipelines.light_pipelines[0]);
        command_buffers[current_frame_index]->bind_vertex_buffers(0, {d_buffers.vertices}, {0});
        command_buffers[current_frame_index]->bind_index_buffers({d_buffers.indices}, 0, VK_INDEX_TYPE_UINT32);
        command_buffers[current_frame_index]->draw_indexed(h_indices.size(), 1, 0, 0, 0);
    ...
    void setup_descriptor_sets() {
        ...
        descriptors.cube_layout->add_binding(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, 1); // Camera uniform buffer
        descriptors.cube_layout->add_binding(VK_SHADER_STAGE_FRAGMENT_BIT, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, 1); // Cube texture sampler
        descriptors.cube_layout->add_binding(VK_SHADER_STAGE_FRAGMENT_BIT, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2, 1); // Light position uniform buffer

        CHECK_RESULT(descriptors.cube_layout->create_layout());
        ev::logger::Logger::getInstance().debug("Cube Descriptor set layout created with handle" +
            std::to_string(reinterpret_cast<uintptr_t>(VkDescriptorSetLayout(*descriptors.cube_layout)))
        );

        for (uint32_t i = 0 ; i < swapchain->get_images().size(); ++i) {
            descriptors.cube_sets.push_back(descriptor_pool->allocate(descriptors.cube_layout));
            ev::logger::Logger::getInstance().debug("Cube Descriptor set " + std::to_string(i) + " allocated");
            descriptors.cube_sets.back()->write_buffer(
                0,
                d_buffers.camera_uniform,
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
            );
            ev::logger::Logger::getInstance().debug("Cube Descriptor set " + std::to_string(i) + " Camera Perspective View uniform buffer written");
            descriptors.cube_sets.back()->write_texture(
                1, 
                d_buffers.texture,
            ...
            ev::logger::Logger::getInstance().debug("Cube Descriptor set " + std::to_string(i) + " Light position uniform buffer written");
            CHECK_RESULT(descriptors.cube_sets.back()->update());
            ev::logger::Logger::getInstance().debug("Cube Descriptor set " + std::to_string(i) + " created");
        }


        descriptors.light_layout = std::make_shared<ev::DescriptorSetLayout>(device);
        descriptors.light_layout->add_binding(VK_SHADER_STAGE_VERTEX_BIT, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, 1); // Light position uniform buffer
        descriptors.light_layout->add_binding(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, 1); // Light view position uniform buffer
        CHECK_RESULT(descriptors.light_layout->create_layout());
        ev::logger::Logger::getInstance().debug("Light descriptor set layout created");
        for (uint32_t i = 0 ; i < swapchain->get_images().size(); ++i) {
            descriptors.light_sets.push_back(descriptor_pool->allocate(descriptors.light_layout));
            ev::logger::Logger::getInstance().debug("Light Descriptor set " + std::to_string(i) + " allocated");
            ...
            ev::logger::Logger::getInstance().debug("Light descriptor set " + std::to_string(i) + " Light position uniform buffer written");
            CHECK_RESULT(descriptors.light_sets.back()->update());
            ev::logger::Logger::getInstance().debug("Light descriptor set " + std::to_string(i) + " created");
        }
    }
    ...
    void setup_shaders() {
        ...
        ev::logger::Logger::getInstance().debug("Vertex shader loaded: " + std::to_string(reinterpret_cast<uintptr_t>(VkShaderModule(*shaders.vertex))));
        ev::logger::Logger::getInstance().debug("Fragment shader loaded: " + std::to_string(reinterpret_cast<uintptr_t>(VkShaderModule(*shaders.fragment))));
        ev::logger::Logger::getInstance().debug("Light Vertex shader loaded: " + std::to_string(reinterpret_cast<uintptr_t>(VkShaderModule(*shaders.light_vertex))));
        ev::logger::Logger::getInstance().debug("Light Fragment shader loaded: " + std::to_string(reinterpret_cast<uintptr_t>(VkShaderModule(*shaders.light_fragment))));
    }

    void setup_pipeline_layouts() {
        // Cube 렌더링 파이프라인
        std::vector<shared_ptr<ev::DescriptorSetLayout>> descriptor_set_layouts = {descriptors.cube_layout};
        std::shared_ptr<ev::PipelineLayout> cube_pipeline_layout 
            = std::make_shared<ev::PipelineLayout>(device, descriptor_set_layouts);
        pipelines.cube_layouts.push_back(cube_pipeline_layout);

        // Light 렌더링 파이프라인
        descriptor_set_layouts = {descriptors.light_layout};
        std::shared_ptr<ev::PipelineLayout> light_pipeline_layout
            = std::make_shared<ev::PipelineLayout>(device, descriptor_set_layouts);
    ...
    void setup_graphics_pipeline() {
        ev::logger::Logger::getInstance().info("[Setup Graphics Pipeline Start]");
        pipelines.pipeline_cache = std::make_shared<ev::PipelineCache>(device);
        ev::GraphicsPipelineBluePrintManager blueprint_manager(device, render_pass);
        vector<shared_ptr<ev::GraphicsPipeline>> cube_pipelines = blueprint_manager.begin_blueprint()
            ...
            .add_vertex_attribute_description(0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, normal))
            .add_vertex_attribute_description(0, 2, VK_FORMAT_R32G32_SFLOAT, offsetof(Vertex, tex_coord))
            .add_vertex_attribute_description(1, 3, VK_FORMAT_R32G32B32A32_SFLOAT,  offsetof(Cube, model) + sizeof(glm::vec4) * 0 )
            .add_vertex_attribute_description(1, 4, VK_FORMAT_R32G32B32A32_SFLOAT,  offsetof(Cube, model) + sizeof(glm::vec4) * 1 )
            .add_vertex_attribute_description(1, 5, VK_FORMAT_R32G32B32A32_SFLOAT,  offsetof(Cube, model) + sizeof(glm::vec4) * 2 )
            .add_vertex_attribute_description(1, 6, VK_FORMAT_R32G32B32A32_SFLOAT,  offsetof(Cube, model) + sizeof(glm::vec4) * 3 )
            ...
            .set_pipeline_layout(pipelines.cube_layouts[0])
            .end_blueprint()
            .create_pipelines(pipelines.pipeline_cache);
        // graphics_pipeline = cube_pipelines[0];
        pipelines.cube_pipelines = cube_pipelines;
        ev::logger::Logger::getInstance().debug("Cube graphics pipeline created successfully with size : " + std::to_string(pipelines.cube_pipelines.size()));

        vector<shared_ptr<ev::GraphicsPipeline>> light_pipelines = blueprint_manager.begin_blueprint()
            .add_shader_stage(shaders.light_vertex)
            .add_shader_stage(shaders.light_fragment)
            .set_vertex_input_state()
            .add_vertex_input_binding_description(0, sizeof(Vertex), VK_VERTEX_INPUT_RATE_VERTEX)
            .add_vertex_attribute_description(0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, pos))
            .add_vertex_attribute_description(0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, normal))
            ...
            .add_color_blend_attachment_state(VK_FALSE)
            .set_depth_stencil_state(VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS_OR_EQUAL)
            .set_pipeline_layout(pipelines.light_layouts[0])
            .end_blueprint()
            .create_pipelines(pipelines.pipeline_cache);

        pipelines.light_pipelines = light_pipelines;
        ev::logger::Logger::getInstance().debug("Light graphics pipeline created successfully with size : " + std::to_string(pipelines.light_pipelines.size()));


        ev::logger::Logger::getInstance().info("[Setup Graphics Pipeline End]");
    }
    ...
    void setup_camera() {
        ...
        camera.view_pos = glm::vec4(0.0f, 0.0f, 8.0f, 1.0f);
    }

    void setup_light() {
        light.pos = glm::vec4(0.0f, 2.0f, 0.0f, 1.0f);
    ...
    void uniform_update() {
        ...
        light.pos = glm::vec4(
            2.0f * cos(current_frame_time), 
            2.0f * sin(current_frame_time), 
            0.0f, 
            1.0f
        );
        light.model = glm::translate(glm::mat4(1.0f), glm::vec3(light.pos));
    ...
    void render() override {
        // fprintf(stdout, "light model : %f %f %f %f\n %f %f %f %f\n%f %f %f %f\n%f %f %f %f", light.model[0][0], light.model[0][1], light.model[0][2], light.model[0][3],
        //     light.model[1][0], light.model[1][1], light.model[1][2], light.model[1][3],
        //     light.model[2][0], light.model[2][1], light.model[2][2], light.model[2][3],
        //     light.model[3][0], light.model[3][1], light.model[3][2], light.model[3][3]);
        // exit(EXIT_SUCCESS);
