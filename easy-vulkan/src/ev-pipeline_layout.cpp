...
PipelineLayout::PipelineLayout(std::shared_ptr<Device> _device,
    ...
    ev_log_info("[ev::PipelineLayout::PipelineLayout] Creating PipelineLayout with device: %llu", static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(device.get())));
    // Create the pipeline layout here
    if (!device) {
        ev_log_error("Invalid device provided for PipelineLayout creation.");
        exit(EXIT_FAILURE);
    }

    if ( layout != VK_NULL_HANDLE ) {
        ev_log_warn("PipelineLayout already created, destroying the old layout.");
        return;
    }
    ...
    ev_log_info("[ev::PipelineLayout::PipelineLayout] Created PipelineLayout successfully.");
}

void PipelineLayout::destroy() {
    if (layout != VK_NULL_HANDLE) {
        ev_log_info("[ev::PipelineLayout::destroy] Destroying PipelineLayout.");
        vkDestroyPipelineLayout(*device, layout, nullptr);
        layout = VK_NULL_HANDLE;
        ev_log_info("[ev::PipelineLayout::destroy] PipelineLayout destroyed successfully.");
    }
}
