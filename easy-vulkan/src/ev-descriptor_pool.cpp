...
DescriptorPool::DescriptorPool(shared_ptr<Device> _device)
    ...
    if (!device) {
        ev_log_error("[ev::DescriptorPool::DescriptorPool] Invalid device provided for DescriptorPool creation.");
        exit(EXIT_FAILURE);
    }
}

void DescriptorPool::add(VkDescriptorType type, uint32_t count) {
    ev_log_debug("[ev::DescriptorPool::add] Adding descriptor type: %s with count : %d", std::to_string(type).c_str(), count);
    ...
    ev_log_debug("[ev::DescriptorPool::add] Added descriptor type: %s with count: %d", std::to_string(type).c_str(), count);
}

VkResult DescriptorPool::create_pool(uint32_t max_sets, VkDescriptorPoolCreateFlags flags) {
    if (!device) {
        ev_log_error("[ev::DescriptorPool::create_pool] Device is null, cannot create descriptor pool.");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    if (pool != VK_NULL_HANDLE) {
        ev_log_warn("[ev::DescriptorPool::create_pool] Descriptor pool already created, destroying the old pool.");
        return VK_ERROR_INITIALIZATION_FAILED;
    }

    if (pool_sizes.empty()) {
        ev_log_error("[ev::DescriptorPool::create_pool] No descriptor types added to the pool.");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ev_log_info("[ev::DescriptorPool::create_pool] Creating descriptor pool with max sets: %d and flags: %d", static_cast<int>(max_sets), static_cast<int>(flags));
    ...
    if (flags == VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT) {
        ...
        if (std::find(extensions.begin(), extensions.end(), "VK_EXT_descriptor_indexing") == extensions.end()) {
            ev_log_error("VK_EXT_descriptor_indexing extension is required for VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT, default flags will be used.");
        }
    }
...
vector<shared_ptr<DescriptorSet>> DescriptorPool::allocates(
    ...
    if ( pool == VK_NULL_HANDLE ) {
            ev_log_error("Descriptor pool is not created, cannot allocate descriptor sets.");
            return {};
    }   
    ...
    if (pool_sizes.empty()) {
        ev_log_error("[ev::DescriptorPool::allocates] No descriptor types added to the pool, cannot allocate descriptor set.");
        return {};
    }
    ai.pNext = nullptr;
    ...
    ev_log_debug("[ev::DescriptorPool::allocates] Allocated %d descriptor sets.", static_cast<int>(layouts.size()));
    return [&] {
...
shared_ptr<DescriptorSet> DescriptorPool::allocate(shared_ptr<DescriptorSetLayout> layout) {
    if (pool == VK_NULL_HANDLE) {
        ev_log_error("[ev::DescriptorPool::allocate] Descriptor pool is not created, cannot allocate descriptor set.");
        return nullptr;
    }

    if (!layout) {
        ev_log_error("[ev::DescriptorPool::allocate] DescriptorSetLayout is null, cannot allocate descriptor set.");
        return nullptr;
    }
    ...
    // if (result != VK_SUCCESS) {
        // ev_log_error(" Failed to allocate descriptor set: " + std::to_string(result));
        // exit(EXIT_FAILURE);
    // }

    ev_log_debug("[ev::DescriptorPool::allocate] Allocated descriptor set");
...
VkResult DescriptorPool::release(shared_ptr<DescriptorSet> descriptor_set) {
    if (!descriptor_set) {
        ev_log_error("DescriptorSet is null, cannot release.");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ...
    if ( result == VK_SUCCESS ) {
        ...
        ev_log_error("Failed to free descriptor set: %d", static_cast<int>(result));
    }
    return result;
}
...
void DescriptorPool::destroy() {
    ev_log_info("[ev::DescriptorPool::destroy] Destroying DescriptorPool.");
    if (pool != VK_NULL_HANDLE) {
        vkDestroyDescriptorPool(*device, pool, nullptr);
        pool = VK_NULL_HANDLE;
        pool_sizes.clear();
    }
    ev_log_info("[ev::DescriptorPool::destroy] DescriptorPool destroyed successfully.");
}
